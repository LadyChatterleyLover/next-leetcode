...
const Jobs = () => {
  ...
  }, [state.postingType, state.chinaCity, state.categoryId, state.subCategoryId])
  ...
  const setPostingType = (type: string | null) => {
    state.postingType = type
  }

  const setChinaCity = (cities: string[]) => {
    state.chinaCity = cities
  }

  const setCategoryId = (ids: string[]) => {
    state.categoryId = ids[0]
    state.subCategoryId = ids[1]
  }
    ...
    <>
      ...
      <div className='flex gap-x-5'>
        <div className='flex-1'>
          ...
          {state.jobList.length || state.jobArticle.length ? (
            <Pagination
        ...
        <div className='w-[256px]'>
          <JobForm setPostingType={setPostingType} setChinaCity={setChinaCity} setCategoryId={setCategoryId}></JobForm>
        </div>
      </div>
    </>
  )
